import { Ajax, Animation, Browser, ChildProperty, Collection, Complex, Component, Event, EventHandler, Internationalization, L10n, NotifyPropertyChanges, Property, SvgRenderer, compile, createElement, extend, isNullOrUndefined, merge, print, remove, setStyleAttribute } from '@syncfusion/ej2-base';
...
class BingMap {
    ...
    getBingMap(tile, key, type, language) {
        let quadKey = '';
        for (let i = this.maps.tileZoomLevel; i > 0; i--) {
            let digit = 0;
            let mask = 1 << (i - 1);
            if ((tile.x & mask) !== 0) {
                digit++;
    ...
    processLayers(layer, layerIndex) {
        ...
        if (!eventArgs.cancel) {
            if (layer.layerType !== 'Geometry') {
                let center = new Point(this.mapObject.centerPosition.longitude, this.mapObject.centerPosition.latitude);
...
let Maps = class Maps extends Component {
    ...
    processRequestJsonData() {
        ...
        this.layersCollection.forEach((layer, layerIndex) => {
            if (layer.shapeData instanceof DataManager) {
                ...
                let dataModule = layer.shapeData;
                let queryModule = layer.query instanceof Query ? layer.query : new Query();
                let dataManager = dataModule.executeQuery(queryModule);
                dataManager.then((e) => {
                    this.processResponseJsonData('DataManager', e, layer);
                });
            }
            else if (layer.shapeData instanceof MapAjax) {
                this.serverProcess['request']++;
                let mapAjax = layer.shapeData;
                let xhr = new XMLHttpRequest();
                xhr.open('GET', mapAjax.url, true);
                xhr.addEventListener('load', () => {
                    this.processResponseJsonData('Ajax', xhr.responseText, layer);
                });
    ...
    processResponseJsonData(type, data, layer) {
        if (type) {
